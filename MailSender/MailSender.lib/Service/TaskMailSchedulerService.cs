...
namespace MailSender.lib.Service
{
    public class TaskMailSchedulerService : IMailSchedulerService
    {
        ...
        public TaskMailSchedulerService(
            IStore<SchedulerTask> TasksStore, IMailService MailService)
        { 
        ...
        public void Start()
        {
            ...
            var scheduler_task = Task.Factory.StartNew(
                SchedulerTaskMethod, TaskCreationOptions.LongRunning);
        }

        public void Stop() => _TaskCancellation.Cancel();

        private async void SchedulerTaskMethod()
        {
        ...
        private async Task Execute(SchedulerTask task)
        {
            ...
            var sender = _MailService.GetSender(server.Address,
                server.Port,server.UseSSL,server.Login,server.Password);
            await sender.SendAsync(task.Sender.Address, task.Recipients.Select(r => r.Address),
